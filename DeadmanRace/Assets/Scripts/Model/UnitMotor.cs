...
public sealed class UnitMotor : IWalk
{
    ...
    {
        _instance = instance;
        _rigidBody = _instance.gameObject.GetComponent<Rigidbody>();
    ...
    private void CharacterMove(float horizontalAxis, float verticalAxis)
    {
        ...
        _rigidBody.AddForce(_instance.transform.forward * _curSpeed);
    }
    private void CharacterRotation(float horizontalAxis)
    {
        if (horizontalAxis != 0.0f)
        {
            if (_curSpeed < 0.1f * _maxSpeed)
                _rigidBody.AddForce(_instance.transform.forward * _rigidBody.mass * 6.0f);
            _instance.rotation *= Quaternion.Euler(0.0f, horizontalAxis * _rotationSpeed, 0.0f);
